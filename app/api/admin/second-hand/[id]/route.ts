...
export async function DELETE(_request:Request,context:{params:Promise<{id:string}>}){const user=await getAdminUser("catalog:write");if(!user)return Response.json({error:"Yetkisiz erişim"},{status:403});const {id}=await context.params,db=getDb();await db.update(secondHandProducts).set({status:"sold",stock:0,updatedAt:new Date()}).where(eq(secondHandProducts.id,id));await db.insert(auditLogs).values({id:crypto.randomUUID(),actorUserId:user.userId,actorEmail:user.email,action:"archive",entityType:"second_hand_product",entityId:id,payload:{}});return Response.json({ok:true})}
